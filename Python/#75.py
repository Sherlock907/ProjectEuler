"""
It turns out that 12 cm is the smallest length of wire that can be bent to form an integer
sided right angle triangle in exactly one way, but there are many more examples.

12 cm: (3,4,5)
24 cm: (6,8,10)
30 cm: (5,12,13)
36 cm: (9,12,15)
40 cm: (8,15,17)
48 cm: (12,16,20)

In contrast, some lengths of wire, like 20 cm, cannot be bent to form an integer
sided right angle triangle, and other lengths allow more than one solution to be found; for example,
using 120 cm it is possible to form exactly three different integer sided right angle triangles.

120 cm: (30,40,50), (20,48,52), (24,45,51)
Given that L is the length of the wire, for how many values of L ≤ 1,500,000 can
exactly one integer sided right angle triangle be formed?
# https://codereview.stackexchange.com/questions/25388/speed-up-solution-to-project-euler-problem-75
# expected result: 161667
"""
from time import perf_counter as p
from math import gcd

s = p()
nmax = 1_500_001

"""
https://en.wikipedia.org/wiki/Pythagorean_triple#Generating_a_triple
a = m2 – n2
b = 2mn
c = m^2 + n^2
"""

nlist = [0]*(nmax+1)
for m in range(2,int((nmax//2)**0.5)):
    for n in range(1,m):
        if (n+m) % 2 != 0 and gcd(n,m) == 1:
            x = 0
            while True:
                x += 1
                a, b, c = x * (m**2 - n**2), x * (2*m*n), x * (m**2 + n**2)
                if (r:=(a+b+c)) <= nmax:
                    nlist[r] += 1
                else:
                    break

print(len([i for i in nlist if i == 1]))
print(round(p()-s,2), "s")